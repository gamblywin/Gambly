...
function publicPost(p,db){const u=db.users.find(x=>x.id===p.authorId);return {...p,author:u?.name||p.author,handle:u?.handle||p.handle,authorAvatar:u?.avatar||''}}
...
async function api(req,res,url){
  ...
  if(url.pathname==='/api/health'&&req.method==='GET')return send(res,200,{ok:true,version:'0.29.0',realtime:true,database:dbStore.getMode(),oauth:Boolean(process.env.GOOGLE_CLIENT_ID),time:new Date().toISOString()});
  if(url.pathname==='/api/ready'&&req.method==='GET'){try{const check=await dbStore.check();return send(res,check.ok?200:503,{ok:check.ok,version:'0.29.0',database:check.mode,...check})}catch(e){return send(res,503,{ok:false,error:'Banco indisponível'})}}
  ...
  if(url.pathname==='/api/sports/live'&&req.method==='GET'){
    ...
    const games=(dbLive.events||[]).filter(e=>e.status==='live').map(e=>{
      const home=(dbLive.teams||[]).find(t=>t.id===e.homeTeamId),away=(dbLive.teams||[]).find(t=>t.id===e.awayTeamId),league=(dbLive.leagues||[]).find(l=>l.id===e.leagueId);
      return {id:e.id,league:league?.name||'Futebol',home:home?.name||'Casa',away:away?.name||'Fora',score:`${e.homeScore??0} - ${e.awayScore??0}`,minute:e.providerEvents?.[e.providerEvents.length-1]?.minute??'',status:'AO VIVO'};
    });
    return send(res,200,{source:'gambly-db',data:games,updatedAt:new Date().toISOString()});
  }
  ...
  if(url.pathname==='/api/predictions'&&req.method==='POST'){
    ...
    const allowed=['winner','draw','double_chance','over_under','both_teams_score','exact_score','first_half_winner','corners_over_under','cards_over_under','shots_on_target_over_under','total_shots_over_under','offsides_over_under','fouls_over_under','team_goals_over_under'];
    if(!event)return send(res,404,{error:'Evento não encontrado.'});
