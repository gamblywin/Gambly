...
export function Header({ user, onLogin, onRegister, onLoggedOut }:{
  ...
  return <header className="topbar">
    ...
    <div className="top-actions">
      <ThemeToggle />
      {user ? <>
        <button className="icon-btn" title="Notificações" onClick={()=>window.location.href='/notifications'}>♧<i className="badge">3</i></button>
        <button className="icon-btn" title="Mensagens" onClick={()=>window.location.href='/messages'}>✉</button>
        <div className="account-wrap">
          <button className="account-btn" onClick={()=>setMenu(v=>!v)} aria-label="Abrir menu da conta">
            ...
            <strong>{user.name.split(' ')[0]}</strong><b>⌄</b>
          </button>
          {menu && <div className="account-menu">
            ...
            <Link className="account-menu-link" href="/profile" onClick={()=>setMenu(false)}>👤 Meu perfil</Link>
            <button onClick={()=>{setMenu(false);window.dispatchEvent(new Event('gambly:profile-edit'))}}>✎ Editar perfil</button>
