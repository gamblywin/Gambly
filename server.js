...
async function api(req,res,url){
  ...
  if(url.pathname==='/api/health'&&req.method==='GET')return send(res,200,{ok:true,version:'0.23.0',realtime:true,database:dbStore.getMode(),oauth:Boolean(process.env.GOOGLE_CLIENT_ID),time:new Date().toISOString()});
  if(url.pathname==='/api/ready'&&req.method==='GET'){try{const check=await dbStore.check();return send(res,check.ok?200:503,{ok:check.ok,version:'0.23.0',database:check.mode,...check})}catch(e){return send(res,503,{ok:false,error:'Banco indisponível'})}}
  ...
  if(url.pathname==='/api/sports/live'&&req.method==='GET'){
    ...
    return send(res,200,{source:'not-configured',data:[],updatedAt:new Date().toISOString(),message:'Configure SPORTS_API_TOKEN ou SPORTMONKS_API_TOKEN para exibir jogos reais. O GAMBLY não apresenta partidas fictícias como se fossem reais.'});
  }
  ...
  if(url.pathname==='/api/feed'&&req.method==='GET'){
    ...
    const posts=all.slice(offset,offset+limit).map(p=>publicPost(p,db));
  ...
  if(url.pathname==='/api/search'&&req.method==='GET'){
    ...
    const posts=db.posts.filter(x=>String(x.title||'').toLowerCase().includes(q)||String(x.text||'').toLowerCase().includes(q)||String(x.market||'').toLowerCase().includes(q)).slice(0,8).map(p=>publicPost(p,db));
    return send(res,200,{users,posts});
  }
  if(url.pathname==='/api/posts'&&req.method==='POST'){if(!user)return send(res,401,{error:'Não autenticado'});if(!rateLimit(req,'post-create',20,60000))return send(res,429,{error:'Limite de publicações atingido. Aguarde um minuto.'});const b=await body(req),p={id:uid('p_'),authorId:user.id,type:b.type||'Análise',title:b.title||'Nova publicação',text:b.text||'',market:b.market||'Mercado',odd:Number(b.odd||0),stake:Number(b.stake||0),confidence:Number(b.confidence||9),likes:0,comments:0,createdAt:new Date().toISOString()};db.posts.unshift(p);user.posts=(user.posts||0)+1;await dbStore.write(db);emit(user.id,'post_update',{postId:p.id,created:true});return send(res,201,{post:publicPost(p,db)});}
...
const server=http.createServer(async(req,res)=>{try{const url=new URL(req.url,`http://${req.headers.host||'localhost'}`);
  ...
  res.setHeader('Content-Security-Policy',"default-src 'self'; img-src 'self' data: https:; style-src 'self' 'unsafe-inline' https://fonts.googleapis.com; font-src 'self' https://fonts.gstatic.com data:; script-src 'self' 'unsafe-inline'; connect-src 'self' https://api.sportmonks.com; form-action 'self'; base-uri 'self'; frame-ancestors 'none';");
...
(async()=>{await dbStore.init();setInterval(()=>{const now=Date.now();for(const [k,v] of sessions)if(v.expiresAt<now)sessions.delete(k);for(const [k,v] of rateBuckets)if(!v.some(t=>now-t<60000))rateBuckets.delete(k);for(const [k,v] of oauthStates)if(v.expiresAt<now)oauthStates.delete(k)},60000);server.listen(PORT,HOST,()=>console.log(`GAMBLY v0.23 rodando em http://${LOCAL_HOST}:${PORT} (escutando em ${HOST}:${PORT}) [DB: ${dbStore.getMode()}]`))})();
